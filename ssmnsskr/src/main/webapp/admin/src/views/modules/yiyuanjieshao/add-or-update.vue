    ...
    >
      ...
      <el-col :span="24">  
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="医院介绍" prop="yiyuanjieshao">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="医院介绍"
                  v-model="ruleForm.yiyuanjieshao" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.yiyuanjieshao" label="医院介绍" prop="yiyuanjieshao">
                    <span>{{ruleForm.yiyuanjieshao}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.dengji = true;
          ...
          if(o=='yiyuanjieshao'){
            this.ruleForm.yiyuanjieshao = obj[o];
	    this.ro.yiyuanjieshao = true;
            continue;
          }
          if(o=='lianxidianhua'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.dengjiOptions = "甲,乙,丙,丁".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `yiyuanjieshao/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yiyuanjieshao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yiyuanjieshaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
