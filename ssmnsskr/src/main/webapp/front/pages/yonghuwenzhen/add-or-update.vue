<template>
	<view class="content">
		<form class="app-update-pv">
				...
				<view :style='{"width":"160rpx","fontSize":"28rpx","color":"rgba(21, 20, 20, 1)","textAlign":"left"}' class="title">问诊时间</view>
				<picker mode="date" :value="ruleForm.wenzhenshijian" @change="wenzhenshijianChange">
					<view :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0.56)","borderColor":"rgba(0,0,0,.6)","borderRadius":"16rpx","color":"rgba(20, 19, 19, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' class="uni-input">{{ruleForm.wenzhenshijian?ruleForm.wenzhenshijian:"请选择问诊时间"}}</view>
				</picker>
			</view>
												<view :style='{"boxShadow":"","backgroundColor":"rgba(255, 255, 255, 0.56)","borderColor":"#ccc","margin":"","borderWidth":"0","borderStyle":"solid","height":"108rpx"}' class="cu-form-group">
...
<script>
	...
	export default {
		...
		async onLoad(options) {
    		        this.ruleForm.wenzhenshijian = this.$utils.getCurDate();
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`yonghuwenzhen`, this.ruleForm.id);
		...
		methods: {
			...
			// 多级联动参数
																																	...
																																	wenzhenshijianChange(e) {
				this.ruleForm.wenzhenshijian = e.target.value;
				this.$forceUpdate();
			},
			...
			async onSubmitTap() {
																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																if(this.ruleForm.id){
					await this.$api.update(`yonghuwenzhen`, this.ruleForm);
				}else{
					await this.$api.add(`yonghuwenzhen`, this.ruleForm);
				}
