	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
        ...
        <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			...
			<view v-if="queryIndex==1" class="search-form round">
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.keshileixing" type="text" placeholder="科室类型" ></input>
			</view>
			<view class="action">
		...
		<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(160, 156, 156, 0.82)"}'>
			<view @tap="onDetailTap(product)" class="uni-product" :style='{"borderRadius":"8rpx","backgroundColor":"#fff"}' v-for="(product,index) in list" :key="index">
					...
					<text v-if="isAuth('yuyueguahao','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
					<text v-if="isAuth('yuyueguahao','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
				</view>
			</view>
		</view>
		
		
		<button :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,0) inset","backgroundColor":"#409EFF","borderColor":"#409EFF","borderRadius":"40rpx","color":"#fff","borderWidth":"1","width":"80%","fontSize":"28rpx","borderStyle":"solid","height":"80rpx"}' v-if="isAuth('yuyueguahao','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.keshileixing){
			...
			async search(){
				...
				let searchForm = {
					...
					searchForm['keshimingcheng'] = '%' + this.searchForm.keshimingcheng + '%'
																...
																if(this.searchForm.keshileixing){
					searchForm['keshileixing'] = '%' + this.searchForm.keshileixing + '%'
				}	
																																																																																																												let res = await this.$api.list(`yuyueguahao`, searchForm);
				// 如果是第一页数据置空
